...
// próxima, como uma barra de progresso contínua. rx/ry via clip-path.
...
export function ChevronFunnel({ stages }: { stages: FunnelStage[] }) {
  ...
  return (
    <div className="flex">
      {stages.map((stage, i) => {
        const bg = stage.isWon ? 'bg-positive-600' : 'bg-brand-700'
        return (
          <div
            key={stage.id}
            className={`flex-1 py-2.5 text-center text-[11px] text-white ${bg} ${i > 0 ? '-ml-2.5 pl-4' : ''}`}
