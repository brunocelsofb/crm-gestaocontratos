...
type Template = { id: string; name: string; subject: string; body: string; trigger_stage_id: string | null }
      ...
      <form ref={formRef} action={formAction} className="space-y-3 rounded-lg border border-dashed border-gray-300 bg-white p-4">
        ...
        <div>
          <label className="block text-xs text-gray-500">
            Corpo do e-mail (HTML simples) — variáveis disponíveis: <code>{'{{cliente}}'}</code> <code>{'{{empresa}}'}</code> <code>{'{{contato}}'}</code> <code>{'{{processo}}'}</code>
          </label>
