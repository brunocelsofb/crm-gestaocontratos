...
export function TimelineFeed({
  ...
  // Separa fixadas das demais — fixadas no topo
  const pinned = events.filter(e => localPins[e.id])
  const rest   = events.filter(e => !localPins[e.id])
  const sorted = [...pinned, ...rest]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <textarea
          ...
          style={{
            width: '100%', padding: '10px 12px', fontSize: 13, borderRadius: 8,
            border: '0.5px solid #d1d8e8', outline: 'none', resize: 'vertical',
            fontFamily: 'inherit', color: '#1a1f36', boxSizing: 'border-box',
          }}
        />
        <button
          onClick={handleAddNote}
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{
          position: 'absolute', left: 14, top: 0, bottom: 0, width: 2,
          background: 'linear-gradient(to bottom, #e8edf5, #f1f3f8)',
          zIndex: 0,
        }} />
            ...
            return (
              <div key={e.id} style={{ display: 'flex', gap: 14, paddingBottom: 20, position: 'relative', zIndex: 1 }}>
                ...
                <div style={{
                  ...
                  border: `2px solid ${isPinned ? '#f59e0b' : cfg.color}22`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 13, flexShrink: 0, zIndex: 2,
                    ...
                    {isNote && (
                      <button
                        ...
                        style={{
                          ...
                          cursor: 'pointer', opacity: pinningId === e.id ? 0.4 : isPinned ? 1 : 0.35,
                          transition: 'opacity 0.15s', flexShrink: 0, padding: 2,
